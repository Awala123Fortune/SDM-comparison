...
for (j in 1:3) {
  ...
  load(file = file.path(FD2, set_no, paste("svm1_", j, "_", dataN[sz], ".RData", sep =
                                             "")))
  
  isNULL <- unlist(lapply(svmf1, is.null))
  save(isNULL, file = file.path(
    PD2,
    set_no,
    paste("svmf1_isNULL_", j, "_", dataN[sz], ".RData", sep = "")
  ))
  
  svm_notnull <- svmf1[!isNULL]
  ...
  for (n in 1:REPs) {
    ...
    probs <-
      foreach (i = 1:length(svm_notnull), .packages = "e1071") %dopar% {
        predict(svm_notnull[[i]], newdata = Xv)
      }
  ...
  save(svm1_PAs, file = file.path(
    PD2,
    set_no,
    paste("svm1_PAs_", j, "_", dataN[sz], ".RData", sep = "")
  ))
