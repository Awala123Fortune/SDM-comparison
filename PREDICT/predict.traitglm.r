...
for (j in 1:3) {
	...
	load(file=file.path(FD2,
	                    set_no,
	                    paste("traitglm1_",j,"_",dataN[sz],".RData",sep="")))
	...
	traitglm1_probs <- predict(traitglm1, newR=Xv)
...
# 	traitglm1_PAs <- (Z1>0)*1
	...
	filebody <- paste0("traitglm1_PAs_", j, "_", dataN[sz])
	if (commSP) {
		filebody <- paste0(filebody, "_commSP")
	}		
	save(traitglm1_PAs, file=file.path(PD2, 
									   set_no, 
