...
for (j in 1:3) {
  ...
  load(file = file.path(FD2, set_no, paste0("gnn1_", j, "_", dataN[sz], ".RData")))
  ...
  gnn1_allProbs[, noPredSp] <-
    colMeans(as.matrix((y_train[[j]][, noPredSp])))
  ...
  tmp1 <- foreach (i = 1:REPs) %dopar% {
          rbinom(gnn1_allProbs, 1, gnn1_allProbs)
  }
  
  gnn1_PAs <- simplify2array(lapply(tmp1, matrix, nrow = nsites, ncol = nsp))
  rm(tmp1)

  if (any(is.na(gnn1_PAs))) {
    stop("Predicted occurrences contain NAs")
  }
  
  filebody <- paste0("gnn1_PAs_", j, "_", dataN[sz])

  save(gnn1_PAs, file = file.path(
    	PD2,
    	set_no,
