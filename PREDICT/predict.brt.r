...
for (j in 1:3) {
	...
	load(file=file.path(FD2,set_no,paste("brt1_",j,"_",dataN[sz],".RData",sep="")))

	isNULL<-unlist(lapply(brt1, is.null))
	save(isNULL, file = file.path(PD2,set_no,paste("brt1_isNULL_",j,"_",dataN[sz],".RData",sep="")))
