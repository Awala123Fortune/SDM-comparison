...
for (j in 1:3) {
	...
	filebody <- paste0("brl1_", j, "_", dataN[sz])
	filebody_comtim <- paste0("comTimes_BORAL1_",dataN[sz])
	
	if (MCMC2) {
		filebody <- paste0(filebody, "_MCMC2")
		filebody_comtim <- paste0(filebody, "_MCMC2")
	}
		
	save(brl1, file=file.path(FD,
							  set_no,
	...
	save(brl2, file=file.path(FD,
							  set_no,
							  paste0(filebody,".RData",sep="")))
	if (j==1) {
		save(comTimes, file=file.path(FD,
									  set_no,
									  paste0(filebody_comtim,".RData")))
	}
