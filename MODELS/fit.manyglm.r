...
for (j in 1:3) {
	...
	if (j==1) {
		eT<-Sys.time()
		comTimes<-eT-sT
		save(comTimes, file=file.path(FD,set_no,paste("comTimes_MVABUND1_",dataN[sz],".RData",sep="")))
	}
	save(manyglm1, file=file.path(FD,set_no,paste("manyglm1_",j,"_",dataN[sz],".RData",sep="")))
