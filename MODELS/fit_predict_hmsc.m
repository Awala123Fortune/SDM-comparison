...
for dTyp=1:3                % interpol, extrapol1, extrapol2
    ...
    for typ=mtypes         % 1=without LF, 2==with LF; 3=with spatial structure as LF

        folder = fullfile(wdpath,'FITS2',set_no);
        if commSP
            folder = fullfile(wdpath,'FITS3',set_no);
        end
        ...
        X_t=importdata(file);
        if intXs
            tmp=x2fx(X_t, 'interaction');
            X_t=[ones(size(tmp,1),1), X_t, tmp(:, 2:end), X_t.^2];
        else
        ...
        X_v=importdata(file);
        if intXs
            tmp=x2fx(X_v, 'interaction');
            X_v=[ones(size(X_v,1),1), X_v, tmp(:, 2:end), X_v.^2];   
        else
        ...
        if commSP
            file=fullfile(folderData,strcat('common_sp_d',num2str(dTyp),'_',num2str(set_no),'.csv'));
            comm_sp=importdata(file);
            Y_t=Y_t(:,comm_sp);
            Y_v=Y_v(:,comm_sp);
        end
    ...
    if intXs
        filebody=strcat(filebody,'_intXs');
        filebodyTime=strcat(filebodyTime,'_intXs');
    end

    if commSP
        filebody=strcat(filebody,'_commSP');
        filebodyTime=strcat(filebodyTime,'_commSP');
    end
    ...
    if intXs
        filebodyPred=strcat(filebodyPred,'_intXs');
        filebodyPredCsv=strcat(filebodyPredCsv,'_intXs');
    end

    if commSP
        filebodyPred=strcat(filebodyPred,'_commSP');
        filebodyPredCsv=strcat(filebodyPredCsv,'_commSP');
    end
